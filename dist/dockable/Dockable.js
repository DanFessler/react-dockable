...
class Dockable extends Component {
  constructor(...args) {
    ...
    _defineProperty(this, "handleContextClick", (actions, x, y) => {
      this.setState({
        contextMenu: {
          show: true,
          position: {
    ...
    _defineProperty(this, "updatePanels", newPanels => {
      this.setState(({
        items
      }) => ({
        panels: newPanels
    ...
    _defineProperty(this, "handlePanelResize", panels => {
      let newPanels = this.getPanels().map((oldPanel, i) => ({ ...oldPanel,
        ...panels[i]
      }));
      this.updatePanels(newPanels);
    });

    _defineProperty(this, "handleWindowResize", (panelId, windows) => {
      let newWindows = this.getPanels()[panelId].windows.map((oldWindow, i) => ({ ...oldWindow,
        ...windows[i]
      }));
    ...
    _defineProperty(this, "handleTabSort", (panelIndex, windowIndex, tabStart, tabEnd) => {
      let newPanels = [...this.getPanels()];
      newPanels[panelIndex].windows[windowIndex].widgets = arrayMove(newPanels[panelIndex].windows[windowIndex].widgets, tabStart, tabEnd);
      this.updatePanels(newPanels);
    });

    _defineProperty(this, "handleTabSelect", (panelId, windowId, tabId, componentId) => {
      let newPanels = [...this.getPanels()];
      newPanels[panelId].windows[windowId].selected = tabId;
      this.updatePanels(newPanels); // this.props.onTabSwitch(this.getSize(panelId, windowId, tabId));

      this.handleOnActive(componentId);
    });

    _defineProperty(this, "handleOnActive", id => {
      if (this.props.onActive) {
        this.props.onActive(id);
      }
    });

    _defineProperty(this, "handleTabClosed", (panelId, windowId, tabId) => {
      ...
      this.updatePanels(newPanels); // if (callback) callback();
    });

    _defineProperty(this, "handleDragStart", result => {
      this.setState({
        draggingTab: true
      });
    });

    _defineProperty(this, "handleDragEnd", result => {
      let newPanels = JSON.parse(JSON.stringify(this.getPanels()));
      let source = result.source.droppableId.split(","); // If we dropped on a tab bar

      if (result.destination && this.state.hoverBorder == null) {
        // remove from source
        let item = newPanels[source[0]].windows[source[1]].widgets.splice(result.source.index, 1); // Select first tab from source window
        ...
        newPanels[destination[0]].windows[destination[1]].selected = result.destination.index;
          ...
          // remove from source
          let item = newPanels[source[0]].windows[source[1]].widgets.splice(result.source.index, 1); // Add to destination
          // If we dropped between windows

          if (this.state.hoverBorder[1] !== null) {
            newPanels[this.state.hoverBorder[0]].windows.splice(this.state.hoverBorder[1], 0, {
              selected: 0,
              widgets: item
            });
          } // If we dropped between panels
          else {
              newPanels.splice(this.state.hoverBorder[0], 0, { ...newPanels[source[0]],
                windows: [{
                  selected: 0,
                  widgets: item
                }]
    ...
    _defineProperty(this, "cleanup", panels => {
  ...
  getFilteredPanels() {
    ...
    return this.props.initialState.filter(panel => {
      return panel.windows.filter(windows => {
  ...
  render() {
    ...
    }, this.getFilteredPanels().map((thisPanel, panelIndex) => /*#__PURE__*/React.createElement(WindowPanel, {
      key: panelIndex,
      index: panelIndex,
      isLast: panelIndex === this.getPanels().length - 1,
      draggingTab: this.state.draggingTab,
      hoverBorder: this.state.hoverBorder,
      onHoverBorder: i => this.setState({
        hoverBorder: i
      }),
